...
class TopkRouting(nn.Module):
    ...
    """
    def __init__(self, qk_dim, topk=4, qk_scale=None, param_routing=False, diff_routing=True):
        super().__init__()
        self.topk = topk
        self.qk_dim = qk_dim
...
class EnhancedBiLevelRoutingAttention(nn.Module):
    """
    n_win: number of windows in one side (so the actual number of windows is n_win*n_win)
    kv_per_win: for kv_downsample_mode='ada_xxxpool' only, number of key/values per window. Similar to n_win, the actual number is kv_per_win*kv_per_win.
    topk: topk for window filtering
    ...
    def __init__(self, dim, num_heads=8, n_win=7, qk_dim=None, qk_scale=None,
                 kv_per_win=4, kv_downsample_ratio=4, kv_downsample_kernel=None, kv_downsample_mode='identity',
                 topk=4, param_attention="qkvo", param_routing=False, diff_routing=True, soft_routing=True, side_dwconv=3,
                 auto_pad=False):
        super().__init__()
        # local attention setting
        self.dim = dim
        ...
        #
        self.mamba_enhancer = SS2D(d_model=self.qk_dim)
        self.norm_mamba = nn.LayerNorm(self.qk_dim)
        ...
        self.norm_q_router = nn.LayerNorm(self.qk_dim)
        self.norm_k_router = nn.LayerNorm(self.qk_dim)
        # softmax for local attention
        self.attn_act = nn.Softmax(dim=-1)
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        B, N_wins, C = k_win.shape
        k_grid = k_win.view(B, self.n_win, self.n_win, C)
        x_norm = self.norm_mamba(k_grid)
        x_permuted = x_norm.permute(0, 3, 1, 2).contiguous()
        k_enhanced_nchw = self.mamba_enhancer(x_permuted)
        k_enhanced = k_enhanced_nchw.permute(0, 2, 3, 1).contiguous()
        k_win = k_win + k_enhanced.view(B, N_wins, C)
        ...
        ############ gather q dependent k/v #################
        # q_win = self.norm_q_router(q_win)
        # k_win = self.norm_k_router(k_win)
        r_weight, r_idx = self.router(q_win, k_win) # both are (n, p^2, topk) tensors
